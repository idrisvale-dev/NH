...
-- Gradient definitions for the floating button
...
Tabs = {
    ...
    GUI_Settings = Window:AddTab({ Title = "GUI Settings", Icon = "solar/settings-bold" }),
}
...
-- Using FluentPro built-in Discord card
secInfo:AddDiscord({
...
-- GUI Settings Tab Content (Improved Layout)
...
-- Fixed: Removed the redundant theme buttons and kept only the InterfaceManager's built-in section 
-- or a single dropdown if needed. Since BuildInterfaceSection already provides theme selection, 
-- we only add custom ones if they aren't in the list.

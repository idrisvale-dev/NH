-- ════════════════════════════════════════════════════════════════════════════
--  NevaHexUI.lua — NevaHex UI Library  (Phase 1)
--
--  Reusable UI library built on embedded FluentPro.
--  Does NOT launch the NevaHex application on load.
--
--  Usage:
--      local Library = loadstring(source)()
--      local Window  = Library:CreateWindow({ Title = "NEVAHEX", ... })
--
--  Managers are accessible as properties on the returned Library object:
--      Library.SaveManager
--      Library.InterfaceManager
--      Library.FloatingButtonManager
--      Library.MediaManager
--
--  External HTTP dependencies (FluentPro icon system, not this library):
--    The FluentPro icon loader fetches icon pack spritesheets on first use
--    from: https://raw.githubusercontent.com/StyearX/Icons/refs/heads/main/
--    Packs: solar, lucide, gravity, sfsymbols, craft, geist, hero, gmi.
--    This is normal FluentPro behaviour; no other external URLs are used.
-- ════════════════════════════════════════════════════════════════════════════

-- ── 1. FluentPro core (embedded) ─────────────────────────────────────────────
...
-- ── FluentPro initialized ─────────────────────────────────────────────────────

-- ── 2. Manager locals ─────────────────────────────────────────────────────────
--    FluentPro's main module attaches these to x (the Fluent object) and also
--    pushes them into getgenv() at runtime. We also expose them here as locals
--    so downstream code within this file can reference them by name.
...
local MediaManager          = Fluent.MediaManager

-- ── 3. NevaHex custom themes ──────────────────────────────────────────────────
--    Registered here at library load time so every consumer gets them
--    immediately via Library:SetTheme("NevahubViolet") etc.
--    NOTE: "NevahubViolet" is preserved as-is for backwards compatibility;
--    renaming it would break any saved configs and existing callers.
...
Fluent:RegisterCustomTheme("NevahubViolet", {
    -- ========================================================
    --  Palette sampled from the NEVAHUB logo mark:
    --    logo magenta-violet core   255,  60, 196
    --    logo bright lilac          190,  85, 255
    --    logo primary violet        150,  35, 235
    --    logo deep indigo end        38,   8,  92
    --  Every surface below is a step on that same ramp, so the
    --  window, sidebar, cards and inputs each read as their own
    --  plane instead of blurring into one dark purple.
    -- ========================================================

    -- brand accent
    Accent = Color3.fromRGB(150, 35, 235),

    -- window shell (deepest plane)
    ...
    -- controls
    -- ToggleSlider is the TRACK colour and ToggleToggled is the FILLED
    -- (on) colour. They must be far apart in brightness or the switch
    -- reads as one solid blob with no visible knob. Track is kept dark
    -- and desaturated; the on-state uses the bright logo lilac so the
    -- two states are unmistakable.
    ...
    -- typography. SubText raised from 190,155,225 (~3.1:1 on these
    -- surfaces) to 198,168,235 (~5.4:1) so it stays readable.
    ...
    -- rim light peaks on the logo magenta so the border looks lit,
    -- not just painted.
    ShineEnabled = true,
    Shine = {
        Speed = 0.5,
    ...
    -- background image kept exactly as it was
    Background = "rbxassetid://133541508207801",
    BackgroundTransparency = 0.12,
...
-- ── 4. API exposure ───────────────────────────────────────────────────────────
--    The Fluent object is the public API.  Managers are already attached to it
--    by FluentPro internals (Fluent.SaveManager, etc.); we ensure the locals
--    above match by asserting before returning so load-time mismatches surface
--    immediately rather than silently.
do
    assert(Fluent ~= nil,
        "[NevaHexUI] FluentPro IIFE returned nil — library failed to initialise.")
    assert(type(Fluent.CreateWindow) == "function",
        "[NevaHexUI] Fluent.CreateWindow missing — FluentPro API broken.")
    assert(type(Fluent.Notify) == "function",
        "[NevaHexUI] Fluent.Notify missing — FluentPro API broken.")
    assert(type(Fluent.RegisterCustomTheme) == "function",
        "[NevaHexUI] Fluent.RegisterCustomTheme missing — theme system broken.")
end

-- ── 5. Return library object ──────────────────────────────────────────────────
return Fluent
